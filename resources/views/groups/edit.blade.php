...
@section('path', 'Groups / Edit')

@section('pageName', 'Edit Group')
...
@section('content')
            ...
            <div class="col-lg-9">
                ...
                <div class="card border-0 shadow-sm rounded-lg">
                    <div class="card-header bg-white border-0 py-4 px-4">
                        <h4 class="mb-0 font-weight-bold text-primary">Edit Group: {{ $group->name }}</h4>
                        <p class="text-muted small">Update the session details, capacity, or location for this group.</p>
                    </div>
                        ...
                        <form action="{{ route('groups.update', $group) }}" method="POST">
                            ...
                            <div class="form-group mb-4">
                                ...
                                <input type="text" name="name" class="form-control form-control-lg border-2 @error('name') is-invalid @enderror" 
                                    placeholder="e.g. Morning Batch - A" value="{{ old('name', $group->name) }}" required>
                            ...
                            <div class="form-group mb-4">
                                ...
                                <select name="program_session_id" class="form-control custom-select border-2 @error('program_session_id') is-invalid @enderror" required>
                                    ...
                                    @foreach($programSessions as $session)
                                        <option value="{{ $session->id }}" {{ old('program_session_id', $group->program_session_id) == $session->id ? 'selected' : '' }}>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-4">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" name="capacity" class="form-control border-left-0 @error('capacity') is-invalid @enderror" placeholder="e.g. 20" value="{{ old('capacity', $group->capacity) }}" required>
                                ...
                                <div class="col-md-6 mb-4">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between p-2 rounded border bg-light">
                                        ...
                                        <div class="custom-control custom-switch">
                                            ...
                                            <input type="checkbox" name="isActive" class="custom-control-input" id="groupActive" value="1" @if(old('isActive', $group->isActive)) checked @endif>
                            ...
                            <div class="mt-5">
                                <button type="submit" class="btn btn-warning btn-block btn-lg shadow rounded-pill font-weight-bold text-white">
                                    <i class="fas fa-sync-alt mr-2"></i> Update Training Group
                                </button>
                            </div>
                        </form>
                    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            // Check if group already has a branch or if 'old' data exists from a validation error
            const existingBranch = "{{ old('branch_id', $group->branch_id) }}";
